...
async def get_summoner(summoner_name: str, tag: str) -> UserDto:
    user = get_and_save_user(summoner_name, tag)
    return user


@app.get("/summoner/by-puuid/{summoner_puuid}")
async def get_summoner_by_puuid(summoner_puuid: str) -> UserDto:
    user = get_and_save_user_by_puuid(summoner_puuid)
    return user


@app.get("/league/{summoner_puuid}")
async def get_summoner_league(summoner_puuid: str) -> List[RanksDto]:
    user_ranks = get_and_save_user_ranks(summoner_puuid)
    return user_ranks


@app.get("/{summoner_puuid}/matches_id")
async def get_user_matches_id(summoner_puuid: str) -> List[str]:
    matches = get_all_user_matches_id(summoner_puuid)
    return matches


@app.get("/match")
async def get_one_match(match_id: str) -> MatchDto | None:
...
async def get_summoner_matches(summoner_puuid: str) -> List[MatchDto]:
    matches = get_user_matches(summoner_puuid)
    return matches


@app.get("/personnages/{summoner_id}")
async def get_summoner_personnages(summoner_id: str) -> List[PersonnageDto]:
    personnages = get_user_personnages(summoner_id)
    return personnages


@app.get("/champion/{championName}")
async def get_champion_infos(championName: str) -> ChampionDto:
