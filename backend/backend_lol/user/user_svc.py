...
def get_and_save_user_ranks(summoner_puuid: str) -> List[RanksDto]:
    ...
    with Session.begin() as session:
        ...
        if (not user_db):
            ...
            logger.info("Test !")
            user_db_solo = (
                session.query(RanksMod).filter((RanksMod.summonerId == user.summonerId), (RanksMod.queueType == "RANKED_SOLO_5x5")).first()
            )
            print(user_db_solo)
            if (not user_db_solo):
            ...
            user_db_flex = (
                session.query(RanksMod).filter((RanksMod.summonerId == user.summonerId), (RanksMod.queueType == "RANKED_FLEX_SR")).first()
            )
            if (not user_db_flex):
