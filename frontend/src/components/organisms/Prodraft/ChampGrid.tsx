...
export default function ChampGrid({ onChampionClick, selectedChampionsBlue, bannedChampionsBlue, selectedChampionsRed, bannedChampionsRed  }: ChampionsListProps){
    ...
    const isInAnyList = (name: string) => {
        return selectedChampionsBlue.includes(name) ||
        ...
        return (
            <div className="rounded bg-gray-900  w-[54%]" >
                ...
                <div className="grid grid-cols-6 gap-4 overflow-y-auto p-4" style={{ maxHeight: '54vh' }}>
                    {filteredChampions.map((champion) => (
                        ...
                        >
                            <div className={`flex flex-col items-center text-white ${isInAnyList(champion.name) ? 'opacity-15' : ''} `}>
                                <img className={" h-[8vh]"} src={champion.icon} alt={champion.name} />
